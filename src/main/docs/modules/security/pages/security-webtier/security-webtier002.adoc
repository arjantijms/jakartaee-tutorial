...
For information on mapping security roles, see <<_mapping_roles_to_users_and_groups_2>>.
...
[NOTE]
An alternative way to perform user authentication, including BASIC and FORM authentication, is to use the `HttpAuthenticationMechanism`, specified by Jakarta Security, and documented in <<_using_jakarta_security>>.
...
For information on setting up the user database, see <<_managing_users_and_groups_in_glassfish_server>>.
